          ...
          {menuItems.map((item) => {
            ...
            return (
              <div
                ...
                className="group flex flex-col items-center px-3 py-2 rounded-full transition-colors duration-200 cursor-pointer"
              >
                <span className={ isActive?  'text-blue-500':'' } >
                  {item.icon}
                </span>

                <span className={`text-sm ${isActive? 'text-blue-500 ': '' }`}>
                  {item.name}
                </span>
              </div>
            );
